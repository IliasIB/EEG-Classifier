...
def simple_lstm_model(filters=1, kernel_size=16, time_window=640):
    ...
    conv1d2 = tf.keras.layers.Conv1D(16, strides=2, kernel_size=kernel_size, activation="relu")(conv1d)
    conv1d_layer = tf.keras.layers.Conv1D(48, strides=2, kernel_size=kernel_size, activation="relu")
    ...
    lstm3 = tf.keras.layers.CuDNNLSTM(48, return_sequences=True)(conv1d2)
    lstm_layer = tf.keras.layers.CuDNNLSTM(48, return_sequences=True)
