...
from experiments.joint_autoencoder_model.model import joint_autoencoder_model
...
if __name__ == "__main__":
    ...
    model = joint_autoencoder_model(time_window=time_window)

    ds_train = ds_creator.prepare("train", batch_equalizer=Encoder2EnvBatchEqualizer(),
                                  batch_size=64, window=time_window)
    ds_validation = ds_creator.prepare("validation",
                                       batch_equalizer=Encoder2EnvBatchEqualizer(),
                                       batch_size=64, window=time_window)
